...
class Pedido():
	...
	@staticmethod
	def productosAcomentar(usuarioActual):
		listaProductosParaComentar = []
		seEncontro=0
		for pedido_actual in Pedido.pedidos:

			if pedido_actual.getEstado()=="Realizado" and pedido_actual.getPersona() == usuarioActual:
				for pedido_producto in pedido_actual.getPedidoProductos():
					producto_actual=pedido_producto.getProducto()
					for ped in listaProductosParaComentar:
						if ped.getId() == producto_actual.getId():
							seEncontro=1
							break
					if seEncontro == 0:
						listaProductosParaComentar.append(producto_actual)
					else:
						seEncontro = 0
		return listaProductosParaComentar
